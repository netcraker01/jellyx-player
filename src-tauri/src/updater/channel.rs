...
//!
//! Build-time injection is preferred: `build.rs` reads `HELIX_INSTALL_CHANNEL`
//! and exposes it via `cargo:rustc-env=HELIX_INSTALL_CHANNEL=...`. The value
//! becomes available at compile time through `env!("HELIX_INSTALL_CHANNEL")`.
//!
...
impl InstallChannel {
    /// Canonical string identifier (matches the `HELIX_INSTALL_CHANNEL` env
...
///
/// Order of precedence:
/// 1. Build-time injected `HELIX_INSTALL_CHANNEL` (when not `"unknown"`).
...
pub fn install_channel(exe_path: Option<&std::path::Path>) -> InstallChannel {
    let baked = env!("HELIX_INSTALL_CHANNEL");
...
fn detect_linux_path(exe_path: Option<&std::path::Path>) -> Option<InstallChannel> {
    ...
    if path_str.starts_with("/usr/") {
        ...
        // because Helix's primary Linux CI target is deb, and the Phase 1
...
mod tests {
    ...
    fn linux_path_heuristic_usr_bin_is_deb() {
        let p = std::path::Path::new("/usr/bin/helix");
    ...
    fn linux_path_heuristic_opt_is_rpm() {
        let p = std::path::Path::new("/opt/helix/helix");
    ...
    fn linux_path_heuristic_home_is_none() {
        let p = std::path::Path::new("/home/user/helix/target/debug/helix");
    ...
    fn install_channel_returns_baked_value_when_known() {
        ...
        let resolved = install_channel(Some(std::path::Path::new("/home/me/helix")));
