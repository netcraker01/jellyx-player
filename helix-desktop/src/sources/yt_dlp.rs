...
//!   local data directory (`{data_local_dir}/helix/bin/yt-dlp[.exe]`).
...
pub fn yt_dlp_command() -> Result<Command, SourceError> {
    ...
    match result {
        Ok(path) => {
            ...
            helix_core::shared::utils::no_window(&mut cmd);
            Ok(cmd)
        }
        Err(e) => Err(e.clone()),
    }
...
///
/// Uses `dirs::data_local_dir()` which resolves to:
/// - Linux: `~/.local/share/helix/bin/`
/// - macOS: `~/Library/Application Support/helix/bin/`
/// - Windows: `C:\Users\{user}\AppData\Local\helix\bin\`
fn managed_bin_dir() -> Result<std::path::PathBuf, SourceError> {
    let data_dir = dirs::data_local_dir().ok_or_else(|| {
        SourceError::DependencyMissing(
            "Cannot determine local data directory for yt-dlp storage".to_string(),
        )
    })?;
    Ok(data_dir.join("helix").join("bin"))
}
...
///
/// Resolution order:
/// 1. **Managed copy**: `{data_local_dir}/helix/bin/yt-dlp[.exe]` — preferred
...
fn ensure_yt_dlp() -> Result<String, SourceError> {
    ...
    if let Ok(path) = managed_bin_path() {
        if path.exists() {
            ...
            let mut cmd = Command::new(&path);
            cmd.arg("--version");
            if let Ok(output) = helix_core::shared::utils::no_window(&mut cmd).output() {
            ...
            eprintln!(
                "[helix] Managed yt-dlp at {} is broken, re-downloading...",
                path.display()
            );
    ...
    // 4. Auto-download
    eprintln!("[helix] No yt-dlp found, auto-downloading...");
    download_yt_dlp()
}
...
fn download_yt_dlp() -> Result<String, SourceError> {
    ...
    eprintln!("[helix] Downloading yt-dlp from {}", url);
    ...
    let output = helix_core::shared::utils::no_window(&mut verify_cmd)
        .output()
        .map_err(|e| {
        SourceError::DependencyMissing(format!(
    ...
    eprintln!("[helix] yt-dlp {} downloaded successfully to {}", version, dest.display());
...
mod tests {
    ...
    fn managed_bin_dir_is_valid() {
        ...
        assert!(dir.to_string_lossy().contains("helix"), "Managed dir should contain 'helix'");
