...
use crate::ipc::dto::{
    ...
    RecommendationItem, SearchFilter, UserPlaylist as UserPlaylistDto,
};
...
/// Return bounded, redacted failure counters for local support diagnostics.
#[tauri::command]
pub fn get_failure_diagnostics() -> crate::observability::FailureDiagnostics {
    crate::observability::failure_diagnostics()
}

/// Read the persisted, explicit opt-in state for remote failure telemetry.
#[tauri::command]
pub fn get_telemetry_settings(
    state: tauri::State<AppState>,
) -> Result<crate::persistence::models::TelemetrySettings, AppError> {
    state.settings.get_telemetry_settings()
}

/// Persist the opt-in state and immediately re-evaluate DSN-gated telemetry.
#[tauri::command]
pub fn set_telemetry_enabled(state: tauri::State<AppState>, enabled: bool) -> Result<(), AppError> {
    state.settings.set_telemetry_enabled(enabled)?;
    crate::observability::configure_remote_telemetry(enabled);
    Ok(())
}

/// Record a redacted HTMLAudio playback outcome. The IPC shape intentionally
/// accepts only bounded timing; URLs, titles, and browser error text stay in
/// the frontend and are never persisted or sent remotely.
fn record_remote_audio_playback(operation: &str, elapsed_ms: u64, succeeded: bool) {
    let elapsed_ms = elapsed_ms.min(60_000);
    crate::observability::record_operation("remote", operation, succeeded);
    crate::observability::record_latency("remote", operation, elapsed_ms);
    if !succeeded {
        crate::observability::expected_failure("remote", operation);
    }
}

#[tauri::command]
pub fn report_remote_audio_playback_failure(elapsed_ms: u64) {
    record_remote_audio_playback("audio_start", elapsed_ms, false);
}

/// Record confirmed HTMLAudio playback start using only bounded elapsed time.
#[tauri::command]
pub fn report_remote_audio_playback_success(elapsed_ms: u64) {
    record_remote_audio_playback("audio_start", elapsed_ms, true);
}

/// Record a failure after HTMLAudio has successfully started.
#[tauri::command]
...
pub fn create_playlist(
    state: tauri::State<AppState>,
    title: String,
) -> Result<UserPlaylistDto, AppError> {
...
pub fn get_playlist_tracks(
    state: tauri::State<AppState>,
    playlist_id: String,
) -> Result<Vec<PlaylistTrackEntryDto>, AppError> {
    let entries = state.playlist.get_playlist_tracks(&playlist_id)?;
    Ok(entries
        .into_iter()
        .map(|e| PlaylistTrackEntryDto {
...
#[tauri::command]
pub fn generate_artist_playlists(
    state: tauri::State<AppState>,
) -> Result<Vec<UserPlaylistDto>, AppError> {
...
pub fn add_artist_favorite(
    ...
    state.playlist.add_artist_favorite_with_source(
        &artist_id,
        src,
        &artist_name,
        thumbnail.as_deref(),
...
pub fn get_all_artist_favorites(
    state: tauri::State<AppState>,
) -> Result<Vec<ArtistFavoriteDto>, AppError> {
    let entries = state.playlist.get_all_artist_favorites()?;
    Ok(entries
        .into_iter()
        .map(|e| ArtistFavoriteDto {
...
pub async fn search_grouped(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        let remote_tracks =
            playback.search_all_tracks_enabled(&query, &enabled, page_offset, page_limit);
        if !remote_tracks.is_empty() {
            // Merge remote tracks into songs if filter allows
            let include_songs =
                parsed_filter.is_none() || parsed_filter == Some(SearchFilter::Songs);
            if include_songs {
            ...
            let include_artists =
                parsed_filter.is_none() || parsed_filter == Some(SearchFilter::Artists);
            if include_artists {
                ...
                for (id, tracks) in remote_artist_map {
                    ...
                    if existing_ids.contains(&id_lower) {
                        ...
                        if let Some(existing) = result
                            .artists
                            .iter_mut()
                            .find(|a| a.id.to_lowercase() == id_lower)
                        {
...
pub async fn get_artist_detail(
    ...
    let remote_tracks =
        tokio::task::spawn_blocking(move || playback.search_all_tracks(&search_name))
            .await
            .map_err(|e| AppError {
                code: "INTERNAL_ERROR".into(),
...
/// Fast for both local and remote tracks because it skips the resolve step.
#[tauri::command]
pub fn play_next_with_track(state: tauri::State<AppState>, track: Track) -> Result<(), AppError> {
...
pub async fn play_stream(
    ...
    let playback = state.playback.clone();
    tokio::task::spawn_blocking(move || playback.play_stream(track, stream_request_id))
        .await
        .map_err(|e| AppError {
            code: "INTERNAL_ERROR".into(),
...
///
/// Returns the absolute path to the cached file (e.g. `/home/user/.local/share/jellyx/youtube_cache/dQw4w9WgXcQ.m4a`).
...
#[tauri::command]
pub fn get_audio_settings(state: tauri::State<AppState>) -> Result<AudioSettingsDto, AppError> {
...
#[tauri::command]
pub fn set_normalize_audio(state: tauri::State<AppState>, enabled: bool) -> Result<(), AppError> {
...
pub fn skip_update_version(
    ...
    state.updater.skip_version(&version).map_err(|e| AppError {
...
pub fn remind_update_later(
    ...
    state.updater.remind_later(&ts).map_err(|e| AppError {
        code: "UPDATE_PREFS_ERROR".into(),
        details: Some(format!("{:?}", e)),
    })
}
...
#[tauri::command]
pub fn open_release_page(app: tauri::AppHandle, url: String) -> Result<(), AppError> {
