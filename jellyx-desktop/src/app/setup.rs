...
use jellyx_core::shared::utils::{ensure_art_cache_dir, migrate_legacy_data_if_needed};
...
pub fn build_app() -> tauri::Builder<tauri::Wry> {
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // idempotent and non-destructive: it skips when the Jellyx directory
            // already exists and never deletes the old `helix/` tree.
            if let Err(e) = migrate_legacy_data_if_needed() {
                eprintln!("[jellyx] warning: failed to migrate legacy data: {:?}", e);
                // Non-fatal: the app can still start with an empty Jellyx state.
            }
